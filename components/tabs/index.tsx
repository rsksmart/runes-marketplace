...
export default function TabsSection({ activeTab }: TabsSectionProps) {
  ...
  return (
    <Tabs className="flex w-full flex-col items-center" value={tab}>
      <TabsList className="grid grid-cols-2 w-fit mb-1">
        <TabsTrigger onClick={() => setTab(Tab.BUY)} value="buy">
          Buy
        </TabsTrigger>
        <TabsTrigger onClick={() => setTab(Tab.SELL)} value="sell">
          Sell
        </TabsTrigger>
      </TabsList>
      <div className="w-full mx-10 justify-center flex">
        ...
        <TabsContent value="sell" className="md:min-w-[400px] max-w-[900px]">
